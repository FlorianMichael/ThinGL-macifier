...
public abstract class SampledTexture extends AbstractTexture {
    ...
    protected SampledTexture(final int glId, final Type type) {
        super(glId, type);
        this.mipMapLevels = de.florianmichael.thingl.GlCommands.get().glGetTextureParameteri(glId, GL43C.GL_TEXTURE_IMMUTABLE_LEVELS); // FlorianMichael - add macOS support
    }

    @Override
    public void refreshCachedData() {
        this.minificationFilter = de.florianmichael.thingl.GlCommands.get().glGetTextureParameteri(this.getGlId(), GL11C.GL_TEXTURE_MIN_FILTER); // FlorianMichael - add macOS support
        this.magnificationFilter = de.florianmichael.thingl.GlCommands.get().glGetTextureParameteri(this.getGlId(), GL11C.GL_TEXTURE_MAG_FILTER); // FlorianMichael - add macOS support
    }
