...
public abstract class RenderBuffer extends GLObject implements ImageStorage {
    ...
    public RenderBuffer() {
        super(de.florianmichael.thingl.GlCommands.get().glCreateRenderbuffers());
    }
    ...
    public static RenderBuffer fromGlIdUnsafe(final int glId) {
        final int samples = de.florianmichael.thingl.GlCommands.get().glGetNamedRenderbufferParameteri(glId, GL30C.GL_RENDERBUFFER_SAMPLES);
        if (samples == 0) {
    ...
    public int getParameterInt(final int parameter) {
        if (!this.parameters.containsKey(parameter)) {
            this.parameters.put(parameter, de.florianmichael.thingl.GlCommands.get().glGetNamedRenderbufferParameteri(this.getGlId(), parameter));
        }
