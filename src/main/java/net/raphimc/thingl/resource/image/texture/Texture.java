...
public abstract class Texture extends GLObject {
    ...
    public Texture(final int target) {
        super(de.florianmichael.thingl.GlCommands.get().glCreateTextures(target));
    }
    ...
    protected void free0() {
        de.florianmichael.thingl.GlCommands.get().glDeleteTextures(this.getGlId());
    }
    ...
    public int getLevelParameterInt(final int level, final int parameter) {
        ...
        if (!parameters.containsKey(parameter)) {
            parameters.put(parameter, de.florianmichael.thingl.GlCommands.get().glGetTextureLevelParameteri(this.getGlId(), level, parameter));
        }
    ...
    protected static int getTextureTarget(final int glId) {
        if (!ThinGL.workarounds().isGetTextureParameterTextureTargetBroken()) {
            return de.florianmichael.thingl.GlCommands.get().glGetTextureParameteri(glId, GL45C.GL_TEXTURE_TARGET);
        } else {
            final int depth = de.florianmichael.thingl.GlCommands.get().glGetTextureLevelParameteri(glId, 0, GL12C.GL_TEXTURE_DEPTH);
            final int samples = de.florianmichael.thingl.GlCommands.get().glGetTextureLevelParameteri(glId, 0, GL32C.GL_TEXTURE_SAMPLES);
            if (samples == 0) {
                if (depth > 1) {
